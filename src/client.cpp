...
std::string InterClient::read_from_server(int filedes)
{
    ...
    if (0 == n)
        throw std::runtime_error("client: " + std::to_string(filedes) + " disconnected");
    if (-1 == n)
        throw std::runtime_error("client: " + std::to_string(filedes) + " timeouted");
...
void InterClient::init_sockaddr(struct sockaddr_in* name, const char* hostname, uint16_t port)
{
